...
public abstract class Captcha extends Randoms {
    ...
    public static final String[] FONT_NAMES = new String[]{"actionj.ttf", "epilog.ttf", "fresnel.ttf", "headache.ttf", "lexo.ttf", "prefix.ttf", "progbot.ttf", "ransom.ttf", "robot.ttf", "scandal.ttf"};
    ...
    protected Color background = Color.WHITE;  // 背景颜色
    protected String format = Constants.PNG_IMG;  // 输出格式
     ...
     * @return 是否成功
     */
    public abstract boolean out(OutputStream os, TextEntry text);
     ...
     * @return base64编码字符串
     */
    public String toBase64(TextEntry text) {
        if (text == null) {
            throw new EasyCaptchaException("the captcha code is not exist!");
        }
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        out(outputStream, text);
        return String.format(Constants.IMAGE_BASE64, format) + Base64.getEncoder().encodeToString(outputStream.toByteArray());
    }
    ...
    public TextEntry createText() {
        char[] alphas = alphas();
        return new TextEntry(alphas, new String(alphas));
    }
