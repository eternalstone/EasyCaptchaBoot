package io.github.eternalstone.captcha.gp.captcha;


import io.github.eternalstone.captcha.gp.base.ChineseCaptchaAbstract;
...
import io.github.eternalstone.captcha.gp.utils.GifEncoder;
...
public class ChineseGifCaptcha extends ChineseCaptchaAbstract {
    ...
    @Override
    public boolean out(OutputStream os, TextEntry text) {
        try {
            if (text == null) {
                throw new EasyCaptchaException("the captcha code is not exist!");
            }
            char[] strs = text.getChars();  // 获取验证码数组
            // 随机生成每个文字的颜色
